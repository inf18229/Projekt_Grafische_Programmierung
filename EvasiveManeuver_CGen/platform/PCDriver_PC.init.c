...
static void initClass_ROOT_PCDriver_PC(ROOT_PCDriver_PC_Class *self)
{
    self->ASCET_TOP_Ptr = (struct L1_PCDriver_PC_Obj*) initInstance_L1_PCDriver_PC();
}

static ASDClassHeader ROOT_ClassHeader_PCDriver_PC = {0, 0, 0, 1, (MethodList *)0, (MethodList *)0};
static ROOT_PCDriver_PC_Class ROOT_PCDriver_PC_ClassObj = {{1, {&ROOT_ClassHeader_PCDriver_PC}, {0}}};

/* Initialization from root node of model */
ASDObject *initializeRoot_PCDriver_PC(void)
{
	struct ROOT_PCDriver_PC_Obj *self = 0;

	initClass_ROOT_PCDriver_PC(&ROOT_PCDriver_PC_ClassObj);
	CREATE_OBJECT(self, ROOT_PCDriver_PC);
...
{
    return initializeRoot_PCDriver_PC();
}
